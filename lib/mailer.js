...
async function sendInvoiceNotification({ businessName, invoice, settings }) {
  ...
  const from = `"Patel Industries Billing" <${fromEmail}>`;
  ...
  // Plain-text fallback (improves deliverability — Gmail penalises HTML-only emails)
  const text = [
    `New Invoice from ${businessName}`,
    '',
    `Invoice #${String(invoiceNumber).padStart(3, '0')}`,
    `Date: ${formatDate(invoiceDate)}`,
    `Buyer / Party: ${buyerName || '—'}`,
    `Grand Total: Rs. ${Number(grandTotal).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
    '',
    'Please find the bill attached as a PDF.',
    '',
    `This is an automated notification from ${businessName} billing system.`,
